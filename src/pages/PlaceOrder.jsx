  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const productId in cartItems) {
        // Get the sizes and quantities for each product
      ...
      switch (method) {
        ...
        case "stripe":
          const responsesStripe = await axios.post(
            backendUrl + "/api/order/stripe",
            orderData,
            { headers: { token } }
          );
          if (responsesStripe.data.success) {
            const { session_url } = responsesStripe.data;
            window.location.replace(session_url);
          } else {
            toast.error(responsesStripe.data.message);
          }
          break;
